    ...
    try {
        ...
        for (const tenant of tenants) {
            ...
            if (daysDiff === 30 || daysDiff === 7) {
                ...
                if (landlord.email) {
                    await sendEmail({ to: landlord.email, subject: messageSubject, html: htmlBody });
                    sentAny = true;
                }
                if (landlord.phone) {
                    await sendSMS({ to: landlord.phone, body: messageBody });
                    sentAny = true;
                }
            }
            ...
            if (daysDiff === 14) {
                ...
                if (tenant.email) {
                    await sendEmail({ to: tenant.email, subject: tenantSubject, html: tenantHtml });
                    sentAny = true;
                }
                if (tenant.phone) {
                    await sendSMS({ to: tenant.phone, body: tenantBody });
                    sentAny = true;
                }
            }
